...
module.exports = (app, db) => {
	...
	app.post("/editpost", (req, res) => {
		...
		if (filter.isProfane(body) || filter.isProfane(subject)) {
			...
			if (camefrom == "webpage") { return res.render("newpost", { "selected": "forum", "title": "CloudClub | Forum", "email": req.session.email, "subject":subject, "profane": true }) } else {
		...
		const query = "UPDATE cloudclub.forum SET body = (?), subject = (?) WHERE postid = (?)";
